...
public class VoucherOrderPublisher {
    ...
    public VoucherOrderPublisher(
            ...
            @Value("${nyc-review.rabbitmq.confirm-timeout:5s}") Duration confirmTimeout,
            @Value("${nyc-review.rabbitmq.replay-batch-size:50}") int replayBatchSize
    ) {
        this(
                order -> {
    ...
    @Scheduled(fixedDelayString = "${nyc-review.rabbitmq.replay-interval-ms:5000}")
