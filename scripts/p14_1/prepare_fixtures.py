#!/usr/bin/env python3
"""Create isolated P14.1 users, auth tokens and one resettable voucher."""
    ...
    statements.extend(
        [
            (
                ...
                f"VALUES ({VOUCHER_ID},1,'P14.1 Load Voucher','Isolated load test only',"
                "'Never use outside the isolated P14.1 environment',100,1000,1,1,"
...
def main() -> int:
    ...
    parser.add_argument(
        "--tokens-output",
        type=Path,
        default=ROOT / "reports/p14-1/runtime/tokens.json",
    )
