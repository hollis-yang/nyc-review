"""Shared, fail-closed helpers for the isolated load-test environment."""
...
COMPOSE_FILE = ROOT / "compose.load-test.yml"
...
class GuardFailure(RuntimeError):
    """Raised before a command could mutate a non-load-test environment."""
...
def validate_isolated_environment(*, require_spring: bool = True) -> dict[str, Any]:
    if COMPOSE_FILE.name != "compose.load-test.yml" or PROJECT != "nyc-review-p14-load":
        raise GuardFailure("Compose identity is not the expected isolated load-test project")
