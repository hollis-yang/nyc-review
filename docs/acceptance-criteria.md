...
- 商户身份来源、字段观测与内容来源必须在后端分开存储。产品 UI 直接展示最终解析结果，不显示置信度、来源或“合成演示”文案；内部数据质量报告仍须区分外部观测与确定性回退。
- Wikimedia 或其他开放许可图片必须包含来源页、作者、许可名称和许可链接。只有严格商户匹配的图片可作为商户专属图；其余图片只能作为分类回退，并在独立 credits artifact 中保留归属信息。
...
- 在线用户创建博客、博客评论或其他内容时，来源只能由服务端写为 `USER_SUBMITTED`；客户端提交的 `sourceType` 不得覆盖该值。来源分类用于内部审计，前端只展示正文与评论层级。
...
1. 用户在 NYC 地图和店铺页浏览真实身份商户及解析后的营业、价格、评分和图片结果，并手动参加秒杀；后端通过 Lua 和可靠异步订单链路完成处理，来源与生成内容分类保留在内部审计数据中。
