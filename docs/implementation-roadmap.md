...
## P13/P13.5/P14 已完成；后续 P15–P17

P10/P11/P11.5/P12/P13/P13.5/P14 已完成。P13 保持 5,000 家真实来源商户身份和 shop ID 不变，修复了“外部评分人数被显示为本地可浏览评论数”的数据契约，重构了评分、评论线程、笔记和笔记评论，并以免费官网/Wikimedia/NYC 来源继续提升图片和商户字段覆盖率。最终数据检查点仍为 `nyc-real-v5-8b645404-m20260824`；100,000 条根评论和 63,500 条回复通过重复、评分、计数与线程门禁，独立 P13 RAG 的 Recall@10 为 99.54%。P13.5 在不改变该数据检查点的前提下完成了前端视觉覆盖。P14 不改数据，完成 Redis Lua/RabbitMQ 可靠性、Agent 并发恢复与 Trace、地图/列表性能、双语和前端回归门禁；18/18 次 Agent 并发运行通过，地图与列表 P95 全部低于目标。后续按 P15 Qdrant Server、P16 发布候选一致性与回滚、P17 项目包装推进；正式范围见 [P10–P17 Delivery Roadmap](p10-p17-roadmap.md)，P14 验收命令与实测结果见 [P14 Stability and Performance Runbook](p14-stability-performance-runbook.md)。
