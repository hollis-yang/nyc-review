...
router = APIRouter()
OwnerSessionHeader = Annotated[
    str,
    Header(alias="x-agent-session", min_length=16, max_length=128),
]
...
async def create_run(
    ...
    request: Request,
    owner_session: OwnerSessionHeader,
    ...
    try:
        return await manager.create(payload, authorization or "", owner_session)
...
async def get_run(
    ...
    snapshot = await request.app.state.agent_runtime.run_manager.get_owned(
        run_id, authorization or "", owner_session
    )
    if snapshot is None:
...
async def stream_run_events(
    ...
    manager = request.app.state.agent_runtime.run_manager
    if await manager.get_owned(run_id, authorization or "", owner_session) is None:
        raise HTTPException(status_code=404, detail="Agent run not found.")
    return StreamingResponse(
        manager.event_stream(run_id, after, authorization or "", owner_session),
...
async def reject_action(
    ...
    try:
        snapshot = await request.app.state.agent_runtime.run_manager.reject_action(
            run_id, action_id, authorization or "", owner_session
        )
    except KeyError:
        raise HTTPException(status_code=404, detail="Agent action not found.") from None
    except ValueError as exc:
